...
test("all seven tools and six resources work through subprocess stdio", { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    for (const [name, args] of fixtures) {
      ...
      if (name === "jev_gate") assert.deepEqual(result.structuredContent, body);
    }
